...
| **D1.3** | RL 训练收敛 demo | 1w | `CartPole 5K 收敛` + `spot 单 leg PPO 50K 收敛`(用 `axon-distributed.DistributedTrainer`) | D1.1, D1.2 |
...
**关键设计**:
- **两层训练路径**:分布式 → `axon-distributed` RLLib(支持多 worker / GPU);单进程 → `stable-baselines3` PPO(已有 `rl` extra)
- CartPole 是 sanity check,真正验收看 spot 单 leg 50K 收敛
- TensorBoard 通过 `tensorboard_log=Path` 参数注入(SB3 原生支持)
...
- 每 trial 跑 50K timesteps,100 trial 8-CPU 预计 1-3h
- TensorBoard 通过 `tensorboard_log=Path` 注入 SB3,每 trial 独立目录
- trial 失败(SB3 NaN / 崩)→ 报告 metric = -inf,sweeper 跳过
